-- ═══════════════════════════════════════════════════════════════════════════
-- 0041 — Tell money REQUESTS apart from money RECEIVED
-- ═══════════════════════════════════════════════════════════════════════════
-- PayPal / Cash App also email when someone REQUESTS money (e.g. a cash-out
-- request sent to our handle). Those must never be matched to a deposit — no
-- money moved — but the admins should still see them, labelled distinctly. The
-- scanner/webhook tag a request with raw.request=true; here we notify it as a
-- request and skip the fill-matching. (Only this new branch differs from 0039.)
...
begin
  ...
  -- A money REQUEST, not a payment: never matches a deposit. Announce it (unless
  -- it's a stale re-scan) with request=true so the alert reads differently.
  if coalesce((p_raw->>'request')::boolean, false) then
    if not coalesce((p_raw->>'stale')::boolean, false) then
      perform notify_admins('payment.detected', 'payment_event', ev_id, jsonb_build_object(
        'matched', false, 'request', true, 'source', p_source, 'amount', p_amount,
