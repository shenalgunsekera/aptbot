-- ═══════════════════════════════════════════════════════════════════════════
-- 0029 — Stripe unmatched payments are silent too (name lives on the receipt)
-- ═══════════════════════════════════════════════════════════════════════════
--
-- A shared Stripe link doesn't tell us WHO paid, so a "payment received" alert
-- can't show a name and just repeats per Stripe event. We already record the
-- amount silently (for one-tap credit auto-link); the single, named, actionable
-- message is the receipt card the player sends. So: crypto AND stripe stay silent
-- when unmatched. PayPal still alerts on unmatched (real money, no request).
...
begin
  ...
  -- Unmatched: crypto and stripe stay silent (recorded for audit / auto-link).
  -- Only PayPal alerts — money in with no request there genuinely needs a look.
  if p_source not in ('crypto', 'stripe') then
