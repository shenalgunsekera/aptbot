-- ═══════════════════════════════════════════════════════════════════════════
-- 0107 — "one cash-out per platform" error points to /addtowithdraw
-- ═══════════════════════════════════════════════════════════════════════════
-- When a player runs /withdraw but already has one in progress on that platform,
-- the refusal now tells them they can grow the existing one instead. Same text
-- for ClubGG and Sportsbook (the % is the platform name). Both bots surface this
-- message, so this one change covers Telegram and Discord. Only the message
-- string changes vs 0036 — the logic is identical.
...
begin
  ...
  -- One open cash-out per platform. Point them to /addtowithdraw so they can grow
  -- the existing one instead of being stuck.
  ...
  if v_open >= 1 then
    raise exception E'you already have a cash out in progress on % — finish or cancel it first.\nIf you would like to add to your withdraw, do /addtowithdraw', pf.name
      using errcode = 'invalid_parameter_value';
  end if;
