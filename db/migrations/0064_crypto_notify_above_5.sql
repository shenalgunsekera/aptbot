-- ═══════════════════════════════════════════════════════════════════════════
-- 0064 — Notify admins on EVERY crypto payment ≥ $5 (matched or not)
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Crypto used to alert only when a detected transfer matched a pending deposit's
-- exact expected amount. But a real crypto deposit rarely lands to the cent —
-- price drift on volatile coins, network fees, the player rounding — so it went
-- UNMATCHED and stayed silent, and the owner never saw the money arrive.
--
-- New rule: any crypto payment of $5 or more alerts the admins, matched or not.
-- Sub-$5 crypto (dust and the zero-value spam transfers that hit public
-- addresses) stays quiet. Non-crypto rails are unchanged. Only the unmatched
-- branch changes vs 0042.
...
begin
  ...
  -- A money REQUEST or a CANCELLED request never matches a deposit.
  ...
  -- Unmatched payment: alert unless stale. Crypto now alerts too, for anything
  -- >= $5 — a real deposit often won't match the exact expected amount. Sub-$5
  -- crypto (dust / zero-value address spam) stays quiet.
