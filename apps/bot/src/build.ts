...
import {
  ...
  addReceipt, addDone, stripeReceipt, cancelDeposit, peerpayBackup, depositSkip, handleStaffReply,
...
export function buildBot(token: string): Bot<Ctx> {
  ...
  bot.callbackQuery(/^dep:skip:(.+)$/, (ctx) => depositSkip(ctx, ctx.match![1]!));
  ...
  bot.on('message:text', async (ctx, next) => {
    ...
    // Staff replying in the admin group to a "payment handle needed" request.
    // Self-gating: only matches a tracked request message, so it's a no-op elsewhere.
    if (await handleStaffReply(ctx)) return;
