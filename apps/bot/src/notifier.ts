...
export function renderNotification(n: Notification): Rendered | null {
  ...
  switch (n.kind) {
    ...
    case 'withdraw.topup_applied':
      return { text: `➕ *${m(p.added, p.currency)} added to your cash-out* — you now have *${m(p.to_pay ?? p.new_total, p.currency)}* still to be paid.\nYou kept your place in line.` };
    ...
    case 'loader.work': {
      ...
      const tag = p.platform ? ` [${md(p.platform)}${p.club ? ` · ${md(p.club)}` : ''}]` : '';
      const who = `${md(p.account ?? p.player_name)}${tag}`;
      return {
        text: reload
    ...
    case 'payment.detected': {
      ...
      if (p.kind === 'sent') {
        return { text: `📤 You sent *${amt}* to *${who}* via *${src}*` };
      }
